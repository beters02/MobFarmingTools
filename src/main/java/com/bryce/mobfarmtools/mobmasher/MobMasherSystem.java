...
import com.bryce.mobfarmtools.spikes.SpikesConstants;
...
public class MobMasherSystem extends EntityTickingSystem<ChunkStore> {
    ...
    public void tick(float dt, int index, @NonNull ArchetypeChunk<ChunkStore> archetypeChunk, @NonNull Store<ChunkStore> store, @NonNull CommandBuffer<ChunkStore> commandBuffer) {
        MobMasherComponent masher = archetypeChunk.getComponent(index, MobMasherComponent.getComponentType());
        if (masher == null) return;

        masher.incrementTicksLifetime(1);
        if (masher.getTicksLifetime() < masher.getTicksPerAction()) return;
        masher.setTicksLifetime(0);
